...
const TrustCard = ({
  ...
  return (
    <div className="max-w-155 w-full md:min-w-145 min-w-72.5 bg-[#FBFBFB] relative overflow-hidden sm:rounded-[24px] rounded-[10px]">
      ...
      <div className="relative sm:p-10 p-4 flex items-center justify-between gap-6 w-full h-full">
        <div className="sm:space-y-6 space-y-2.5 max-w-83.25 w-full">
          <p className="bg-white rounded-[36px] px-2 py-0.75 font-medium sm:text-sm text-xs tracking-[-2%] w-fit">
            {id}
          </p>
          <h6 className="font-exo xl:text-[24px] lg:text-lg sm:text-base text-sm font-bold text-[#18A154]">
            {title}
          </h6>
          <p className="max-sm:text-sm sm:font-medium tracking-[-3%]">{desc}</p>
        </div>
